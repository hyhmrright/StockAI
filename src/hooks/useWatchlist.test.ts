...
describe("useWatchlist", () => {
  ...
  it("add 添加新 symbol 到列表末尾", async () => {
    ...
    await act(async () => { result.current.add("GOOG"); });
  ...
  it("add 重复 symbol 时不添加", async () => {
    ...
    // AAPL 已在默认列表中
    await act(async () => { result.current.add("AAPL"); });
  ...
  it("remove 不存在的 symbol 时列表不变", async () => {
    ...
    await act(async () => { result.current.remove("NONEXIST"); });
  ...
  it("add 后持久化到 store", async () => {
    ...
    await act(async () => { result.current.add("AMZN"); });
  ...
  it("remove 后持久化到 store", async () => {
    ...
    await act(async () => { result.current.remove("TSLA"); });
