...
- **Portfolio Tracking**: Record real positions (shares + weighted-average cost) and see live market value, unrealized P&L, day P&L and position weight at a glance; adding to a holding merges the cost basis by share-weighted average. **Currencies are summarized separately** — this app has no FX data source, so combining CNY and USD would produce a meaningless number. Positions without a quote are excluded from the totals and named explicitly, never filled in with zeros.
- **Market Overview**: A persistent index bar (SSE / SZSE / ChiNext / S&P 500 / Nasdaq / Dow) that expands into industry and concept sector leaderboards (main-force net inflow, advancers/decliners, leading stock) plus the **Dragon-Tiger list** — the latest session's top net buys and sells with each listing reason.
- **Company Profile (F10, A-shares)**: Corporate overview, latest-period revenue breakdown by product / industry / region with gross margins, holder-account count and top-10 tradable shareholders, and sector tags. Collapsed by default; fetched only when expanded.
