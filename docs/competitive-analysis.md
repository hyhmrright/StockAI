...
§C 表中 **#10 的剩余部分已落地**，另补上一块该表完全没列的空白（组合/持仓）：

| §C# | 缺口 | 本次状态 | 要点 |
|---|---|---|---|
| #10 | A 股特色数据（资金流/概念板块/龙虎榜/F10） | **本项已填平** | 板块榜：东财 push2 clist（`m:90 t:2` 行业 / `t:3` 概念）。龙虎榜：`RPT_DAILYBILLBOARD_DETAILSNEW`，先探最新交易日再过滤（该表约 8.8 万页，不过滤会静默返回 2015 年数据）。F10：emweb `PC_HSF10` 四个端点 allSettled 并发，24h 缓存。资金流原本就有（`quant/fundflow.ts`） |
| — | **大盘视角**（该表未列） | 已做 | 指数条：上证/深证/创业板/标普/纳指/道指。实测这些代码走既有报价链路即可，零新增数据源 |
| — | **用户真实持仓**（该表未列，是"个股分析器→炒股软件"的关键一环） | 已做 | SQLite `positions` 表 + 分币种汇总。**跨币种不合并**（无汇率源）、未定价持仓不进汇总 |

同期修掉两个**数字算错**类缺陷（金融软件里杀伤力最大的一类）：回测胜率此前比的是买卖
两端价格、没扣手续费，凡毛涨幅低于约 0.2% 的单子实际亏损却被记成盈利单；回测净值曲线
终点漏扣平仓费，与报告的总收益率对不上。两者此前都无测试看守。

**仍未做**：#11 财报 RAG 反哺历史财务（部分已做）、#12 大师因子预计算（部分已做）。
#9 inline 溯源与 #6/#7 图表叠加已在此前会话完成。

**接 #10 时踩到的两个坑**（加新 A 股数据源前先看一眼，细节见 `.claude/rules/architecture.md`）：

1. 东财 clist 系列不带 `np=1` 时 `data.diff` 返回对象而非数组，解析静默得到空表。
2. datacenter 的 `reportName` 多是全历史表，不带日期 `filter` 时返回的是十年前的数据——
   形状完全合法，因此只有「断言日期够近」的集成测试能发现。

顺带修掉一个同类隐性缺陷：**北交所 920 号段被识别成美股**（代码识别只认 6/0/3/4/8 开头，
920 是 2024 年启用的新号段）。实测东财北交所板块按代码序前 5 只全是 920xxx，选股选中后
点开必然空白——即「A 股原生」这条护城河上一个真实的漏水点。
