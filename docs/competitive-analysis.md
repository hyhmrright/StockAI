...
| §C# | 缺口 | 本次状态 | 要点 |
|---|---|---|---|
| #10 | A 股特色数据（资金流/概念板块/…） | **概念+行业板块榜已做** | 东财 push2 clist（`m:90 t:2` 行业 / `t:3` 概念），含主力净流入、涨跌家数、领涨股。**龙虎榜与 F10 仍缺**（龙虎榜端点已实测可用：`RPT_DAILYBILLBOARD_DETAILSNEW`，需按最新交易日过滤） |
...
**仍未做**（原判定依然成立）：#11 财报 RAG 反哺历史财务（部分已做）、#12 大师因子预计算
（部分已做）、龙虎榜/F10。#9 inline 溯源与 #6/#7 图表叠加已在此前会话完成。
