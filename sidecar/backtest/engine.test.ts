...
function generateTrendingKline(days: number, startPrice: number, endPrice: number): KlinePoint[] {
  ...
  for (let i = 0; i < days; i++) {
    ...
    points.push({
      ...
      open: price, high: Math.max(price, close) * 1.005,
      low: Math.min(price, close) * 0.995, close, volume: 1000000,
    });
    price = close;
  }
  return points;
...
describe('runBacktest', () => {
  test('returns result with equity curve', () => {
    ...
    const result = runBacktest(kline, {
      symbol: 'TEST', period: 200, buyThreshold: 55, sellThreshold: 45,
      initialCapital: 100000, transactionCost: 0.001,
    });
  ...
  test('returns zero trades with extreme thresholds', () => {
    ...
    const result = runBacktest(kline, {
      symbol: 'TEST', period: 100, buyThreshold: 99, sellThreshold: 1,
      initialCapital: 100000, transactionCost: 0.001,
    });
  ...
  test('rejects insufficient data', () => {
    ...
    const result = runBacktest(kline, {
      symbol: 'TEST', period: 20, buyThreshold: 60, sellThreshold: 40,
      initialCapital: 100000, transactionCost: 0.001,
    });
    expect(result.totalTrades).toBe(0);
    expect(result.equityCurve.length).toBeGreaterThan(0);
